...
/* static ParserNode *parse_block(Parser *p) {
    PRINT_FUNC_NAME;

    ParserNode *node = new_node(p, NODE_BLOCK);

    // Peek ahead to determine closure parameters e.g. { id => or { id,id =>
    b8 is_closure = false;
    if (peek(p)->type == T_IDENT) {
        // Save the current position and scan forward
        size_t saved_pos = p->pos;

        // Consume identifiers separated by commas
        while (peek(p)->type == T_IDENT) {
            advance(p);
        ...
        if (peek(p)->type == T_EQUALS_RARROW) {
            is_closure = true;
        }
    ...
    if (is_closure) {
        ...
        while (peek(p)->type == T_IDENT) {
            ...
            params[params_count++] = (Param){
                .name = "", .type = {0}}; // TODO: Default type? and name
            if (peek(p)->type == T_COMMA)
                advance(p);
            else
                break;
        ...
        expect(p, T_EQUALS_RARROW, "Expected '=>' after block parameters.");

        node->block.params = params;
        node->block.params_count = params_count;
    }

    // Parse the statements then until the closing brace
    ParserNode **stmts = NULL;
    size_t count = 0;

    // Note we are duplicating some logic here from the function stuff
    // which is fine but need to ensure the logic is the same
    // REF: parse_func
    while (peek(p)->type != T_RBRACE && peek(p)->type != T_END) {
        if (match(p, T_NEWLINE))
            continue;
        stmts = realloc_array(p->arena, stmts, ParserNode *, count + 1);
        stmts[count++] = parse_stmt(p);
    }

    expect(p, T_RBRACE, "Expected '}' after block.");
